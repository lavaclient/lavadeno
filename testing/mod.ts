import { startBot, cache, ws, Embed, CreateMessage, sendMessage, getUser } from "https://deno.land/x/discordeno/mod.ts";
...
cluster.on("nodeConnect", (node, reconnect) => {
    ...
    node.rest.routePlanner
        .status()
        .then(status => console.dir(status));
});
        ...
        messageCreate: async message => {
            ...
            switch (command.toLowerCase()) {
                case "join": {
                    ...
                    if (player?.connected) {
                        return message.reply(embed("A player for this guild already exists"), false)
                    }
                ...
                case "nowplaying": {
                    ...
                    if (!player?.connected) {
                        return message.reply(embed("A player for this guild doesn't exist."), false)
                    }
                    ...
                    return message.reply(embed(`**Duration:** ${ms(current.length, { long: true })}`, {
                        title: current.title,
                        url: current.uri,
                        footer: requester
                ...
                case "nightcore": {
                    ...
                    player.filters.timescale = player.filters.timescale?.rate !== 1
                        ? { rate: 1.09, pitch: 1.125, speed: 1 }
                        : { rate: 1, pitch: 1, speed: 1 }

                    player.setFilters();

                    return message.reply(embed(`${player.filters.timescale?.pitch === 1 ? "Disabled" : "Enabled"} **nightcore**!`), false);
                }
                case "play": {
                    const vc = message.guild?.voiceStates?.get(message.authorId);
                    if (!vc?.channelId) {
                    ...
                    const results = await cluster.rest.loadTracks(/^https?:\/\//.test(query)
                        ? query
                        : `ytsearch:${query}`);

                    let tracks: Lavalink.Track[] = [], msg = "";
                    ...
                    const queue = queues.get(player.guildId) ?? createQueue(player, message.channelId);
...
function createQueue(player: Player, channelId: bigint): Queue {
    ...
    queue.on("trackStart", song => {
        sendMessage(channelId, embed(`Now playing [**${song.title}**](${song.uri}) <@${song.requester}>`));
    });
